...
if (!envPath) {
  logger.warn(`.env файл не найден. Проверены пути: ${candidates.join(' | ')}`)
} else {
  logger.info(`Использую .env: ${envPath} (размер: ${fileSize} байт)`) 
}
...
const loadEnvFile = (filePath, override = true) => {
  ...
  logger.info(`Загружены переменные окружения из .env: ${count}`)
  if (count === 0) {
    logger.warn('В .env не найдено ни одной пары KEY=VALUE. Проверь содержимое и кодировку файла.')
  }
  return { loaded: count }
}
...
try {
  ...
  logger.error('Ошибка валидации переменных окружения:', error.flatten().fieldErrors)
  process.exit(1)
}

if (!validatedEnv.BOT_TOKEN) {
  logger.warn('BOT_TOKEN не задан. Бот не будет запущен (сервер поднимется без него).')
}
